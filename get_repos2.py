...
if __name__ == "__main__":
    ...
    # 🔹 Filtro: manter apenas repositórios com pelo menos 100 PRs (MERGED + CLOSED)
    repos_with_enough_prs = {
        repo_name: prs
        for repo_name, prs in repo_pr_map.items()
        if len(prs) >= 100
    }

    filtered_repos = [
        repo for repo in repos
        if repo["node"]["nameWithOwner"] in repos_with_enough_prs
    ]

    print(f"Filtered to {len(filtered_repos)} repositories with >= 100 PRs.")

    # Save the filtered repositories and their pull requests to a JSON file
    save_repos_and_prs_to_json(filtered_repos, repos_with_enough_prs, "repos_and_prs.json")
    print("Data saved to repos_and_prs.json (only repos with >= 100 PRs).")
